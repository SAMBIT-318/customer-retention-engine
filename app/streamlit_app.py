    ...
    if st.button("Predict Risk", type="primary"):
        if model is None:
            ...
            input_data = pd.DataFrame({
                ...
                "Contract": [contract_mapping[contract]], "TechSupport": [support_mapping[tech_support]]
            })
...
# ==========================================
# TAB 4: AI RETENTION STRATEGIST (NEW!)
# ==========================================
with tab4:
    ...
    # Securely ask for the API key in the UI
    ...
    if st.button("✨ Generate Retention Plan", type="primary"):
        if not api_key:
            st.warning("⚠️ Please enter your Gemini API Key in the box above to use this feature.")
        else:
            try:
                # Connect to the AI
                ...
                # The exact prompt we send to the AI
                prompt = f"""
